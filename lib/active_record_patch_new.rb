	...
	class Base	
		class << self
			...
			def method_missing_with_associations(method_id, *arguments, &block)
		      ...
		      return method_missing_without_associations(method_id, arguments, &block) unless all_fields_exist?(field_names)
		      ...
		      if match.scope?
		          ...
		          def self.#{method_id}(*args)                                     # def self.scoped_by_region_id(*args)
		            ...
		            scope_hash = construct_scope_options_from_association_names(   #   scope_hash = construct_scope_options_from_association_names(
		              [:#{all_fields_exist.join(',:')}], args, options            #     [:region_id, :function_id], args
		            )                                                              #   )
			...
			def find_reflection(association_name)
				reflections[association_name.to_sym] || reflections[association_name.to_s.pluralize.to_sym]
			end
			...
			def construct_scope_options_from_association_names(association_names, arguments, options = {})
				includes = association_names.map { |name| extract_association(name).to_sym }
				...
				association_names.each_with_index do |name, idx|
					...
					table_name = use_join_table ? ( [:has_many, :has_and_belongs_to_many].include?(reflection.macro) ? reflection.options[:join_table] + "." : "" ) :
			...
			def construct_scope_options_for_association_field(field_name, argument, options = {})
					raw_field_name = raw_field_name.to_s
					association_name = extract_association(name)
					
					reflection = find_reflection(association_name)
					
					use_join_table = raw_field_name.include?("_id")
					table_name = use_join_table ? ( [:has_many, :has_and_belongs_to_many].include?(reflection.macro) ? reflection.options[:join_table] + "." : "" ) :
																	 association_name.pluralize + "."
					
					# the field name is either the given name (when a name followed by _id is given, or it is a hash)
					field_name, prepared_arguments[idx] = (use_join_table ? [raw_field_name, arguments[idx] ] : options.to_a.flatten[idx*2..idx*2+1] )
					...
					if field_name.include?("like_")
						field_name = field_name.gsub("like_", '')                              # remove the like_ prefix
						prepared_arguments = "%" + prepared_arguments + "%"          # add wildcards
						use_like = true
					end
				...
				[prepared_conditions.join(" AND "), *prepared_arguments], :include => includes}
				
			end
			
			def construct_scope_options_from_attributes_and_association(names, arguments, options = {})
				includes = []
				prepared_conditions = []
				prepared_arguments = []
				
				names.each_with_index do |name, idx|
					if attribute_exist?(name)
